...
Secrets for `hacker-house-medellin/hhm-mash-web` are **committed, encrypted**, with [sops] + [age],
...
- **No multi-line values.** A PEM must be a single line with `\n` escapes:
  `JWT_PRIVATE_KEY="[PEM private key omitted]"`
...
```sh
just env-docker-run dev ghcr.io/hacker-house-medellin/hhm-mash-web:dev
